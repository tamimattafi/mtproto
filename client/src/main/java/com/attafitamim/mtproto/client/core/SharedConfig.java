...
import com.attafitamim.mtproto.client.tgnet.SerializedData;
...
public class SharedConfig {
    ...
    public static void loadConfig(Context context) {
        synchronized (sync) {
            ...
            preferences = context.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
    ...
    public static void loadProxyList(Context context) {
        if (proxyListLoaded) {
            return;
        }
    ...
    public static void saveProxyList(Context context) {
        SerializedData serializedData = new SerializedData();
        int count = proxyList.size();
        serializedData.writeInt(count);
        for (int a = 0; a < count; a++) {
            ProxyInfo info = proxyList.get(a);
            serializedData.writeString(info.address != null ? info.address : "");
            serializedData.writeInt(info.port);
    ...
    public static ProxyInfo addProxy(ProxyInfo proxyInfo, Context context) {
        loadProxyList(context);
        int count = proxyList.size();
        for (int a = 0; a < count; a++) {
            ProxyInfo info = proxyList.get(a);
        ...
        saveProxyList(context);
        return proxyInfo;
    }

    public static void deleteProxy(ProxyInfo proxyInfo, Context context) {
    ...
    public static void checkSaveToGalleryFiles() {
        try {
            File telegramPath = new File(Environment.getExternalStorageDirectory(), "Chat EOS");
            File imagePath = new File(telegramPath, "Chat EOS Images");
            imagePath.mkdir();
            File videoPath = new File(telegramPath, "Chat EOS Video");
            videoPath.mkdir();

...
public class StatsController {
    ...
    public static StatsController getInstance(int num, Context context, File filesDir) {
        StatsController localInstance = Instance[num];
        if (localInstance == null) {
            synchronized (StatsController.class) {
                localInstance = Instance[num];
                if (localInstance == null) {
                    Instance[num] = localInstance = new StatsController(num, context, filesDir);
                }
            }
        }
    ...
    private StatsController(int account, Context context, File filesDir) {
        if (account != 0) {
            filesDir = new File(filesDir, "account" + account + "/");
            filesDir.mkdirs();
        }
        ...
        if (needConvert) {
            ...
            if (account == 0) {
                ...
                sharedPreferences = context.getSharedPreferences("stats" + account, Context.MODE_PRIVATE);
            }
            boolean save = false;
            for (int a = 0; a < 3; a++) {

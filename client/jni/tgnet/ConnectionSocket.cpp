...
int32_t ConnectionSocket::checkSocketError(int32_t *error) {
    ...
    if (ret != 0 || code != 0) {
        ...
        std::string error = std::string("socket error 0x%x code 0x%x", ret, code);
//        __android_log_print(ANDROID_LOG_VERBOSE, "errorSocket!!!!!", error.c_str(), 1);
    }
    *error = code;
    return (ret || code) != 0;
}
...
//    __android_log_print(ANDROID_LOG_VERBOSE, "Socket!!!!!", "closeSocket", 1);
    lastEventTime = ConnectionsManager::getInstance(instanceNum).getCurrentTimeMonotonicMillis();
